...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("Gᴏ ᴛᴏ ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 Pages {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
        ...
        btn.append(
            [
                InlineKeyboardButton("Gᴏ ᴛᴏ ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"),
                InlineKeyboardButton(f"🗓 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"),
                InlineKeyboardButton("Gᴏ ᴛᴏ ɴᴇxᴛ", callback_data=f"next_{req}_{key}_{n_offset}")
            ],
        )
    try:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            InlineKeyboardButton('♻️Group', url='https://t.me/cinemahubmoviesS'),
            InlineKeyboardButton('♥️ Source', url='https://t.me/MyGithubSorceCode')
            ],[
            InlineKeyboardButton('🏠 Home', callback_data='start'),
